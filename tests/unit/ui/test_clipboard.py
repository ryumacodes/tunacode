...
from tunacode.ui.clipboard import (
    _copy_osc52,
    _copy_pbcopy,
    _copy_to_clipboard,
    _copy_wl_copy,
...
def test_copy_selection_to_clipboard_multiple_widgets(mock_app: MagicMock) -> None:
    ...
    with patch("tunacode.ui.clipboard._copy_to_clipboard") as mock_copy_to_clipboard:
        result = copy_selection_to_clipboard(mock_app)

        assert result == "first selection\nsecond selection"
        mock_copy_to_clipboard.assert_called_once_with("first selection\nsecond selection")
        mock_app.notify.assert_called_once_with(
...
def test_copy_selection_to_clipboard_preview_shortening(mock_app: MagicMock) -> None:
    long_text = "a" * 100
    widget = MockWidget(text_selection=SimpleNamespace(), get_selection_result=(long_text, None))
    mock_app.query.return_value = [widget]
...
def test_copy_to_clipboard_stops_after_verified_copy() -> None:
    ...
    with (
        ...
        patch("tunacode.ui.clipboard._read_clipboard", return_value="hello"),
    ):
...
def test_copy_to_clipboard_raises_when_all_strategies_raise() -> None:
    ...
    with (
        patch("tunacode.ui.clipboard._COPY_METHODS", [mock_osc52, mock_pyperclip]),
...
def test_read_clipboard_returns_first_successful_reader() -> None:
    ...
    with patch("tunacode.ui.clipboard._READ_CLIPBOARD_METHODS", [mock_reader, mock_reader2]):
...
def test_read_clipboard_falls_through_on_failure() -> None:
    ...
    with patch("tunacode.ui.clipboard._READ_CLIPBOARD_METHODS", [failing]):
...
def test_read_clipboard_skips_failing_reader() -> None:
    ...
    with patch("tunacode.ui.clipboard._READ_CLIPBOARD_METHODS", [failing, working]):
        assert _read_clipboard() == "hello"
...
def test_copy_methods_includes_available_commands() -> None:
    ...
    import tunacode.ui.clipboard as mod
...
async def test_ui_clipboard_notification_does_not_crash_on_markup_text() -> None:
    app = TextualReplApp(state_manager=StateManager())

    async with app.run_test(headless=True, notifications=True) as pilot:
        await app.mount(ClipboardSelectionWidget("[/]"))
        with patch("tunacode.ui.clipboard._copy_to_clipboard"):
...
async def test_ctrl_y_and_ctrl_shift_c_trigger_copy_action() -> None:
    app = TextualReplApp(state_manager=StateManager())

    async with app.run_test(headless=True) as pilot:
        with patch("tunacode.ui.app.copy_selection_to_clipboard") as mock_copy:
            await pilot.press("ctrl+y")
            await pilot.press("ctrl+shift+c")

    assert mock_copy.call_count == 2
    mock_copy.assert_any_call(app, show_toast=False)
