...
def test_tool_name_enum_exists():
    ...
    expected_tools = [
        "read_file", "write_file", "update_file", "run_command", 
        "bash", "grep", "list_dir", "glob", "todo"
    ]
...
def test_tool_categorization_uses_enums():
    ...
    from tunacode.constants import READ_ONLY_TOOLS, WRITE_TOOLS, EXECUTE_TOOLS, ToolName
...
def test_todo_priority_enum_exists():
    ...
    for priority in expected_priorities:
        assert hasattr(TodoPriority, priority.upper()), f"TodoPriority should have {priority.upper()}"
