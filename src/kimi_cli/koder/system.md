...
agent: [tool_call: list files for path '.']
```

```
user: start the server implemented in server.js
agent: [tool_call: shell command 'node server.js &' because it must run in the background]
```
...
agent: Okay, I can refactor 'src/auth.py'.
...
agent: I can run `rm -rf ./temp`. This will permanently delete the directory and all its contents.
```
