    ...
    def _init_undo_directory(self):
        ...
        try:
            ...
            if not readme_path.exists():
                readme_path.write_text("""# TunaCode Undo System
...
""")
    ...
    def get_undo_status(self) -> Dict[str, any]:
        ...
        try:
            ...
            return {
                "enabled": False,
                "error": "Cannot access undo directory"
            }
    ...
    async def safe_write(self, filepath: Path, content: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            self.undo._log_operation({
                ...
                "size": len(content),
                "backup": str(backup_path.relative_to(self.undo.undo_dir)) if backup_path else None,
                "timestamp": datetime.now().isoformat()
            })
