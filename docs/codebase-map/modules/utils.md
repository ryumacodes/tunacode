...
exports: [estimate_tokens, estimate_message_tokens, estimate_messages_tokens, parse_json]
seams: [M]
---
...
**token_counter.py**
- **estimate_tokens()** - Token counting for plain text (length heuristic)
- **estimate_message_tokens()** - Token counting for single messages
- **estimate_messages_tokens()** - Token counting for message lists
- Uses CHARS_PER_TOKEN=4 heuristic
- Used for context window management

Note: Previous caching layer (MessageTokenCache) was removed because message
formats are in flux during pydantic-ai removal. Will revisit when stabilized.
